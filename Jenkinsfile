            ...
            parallel{
                ...
                stage('Plugin Scan'){  // Scans the built image using the Sysdig Jenkins Plugin
                    steps{
                        script{
                            if(env.sysdig_plugin){
                                sysdigImageScan engineCredentialsId: 'sysdig-sa-credentials', imageName: "${registry_url}/${registry_repo}/${docker_tag}", engineURL: "${params.sysdig_url}", policiesToApply: "${params.plugin_policies_to_apply}", bailOnFail: "${params.bail_on_fail}", bailOnPluginFail: "${params.bail_on_plugin_fail}", cliVersionToApply: "1.4.0", cliVersionToApply: "1.4.0"
                            }
                            else{
                                echo 'Using CLI Scan'
                            }
