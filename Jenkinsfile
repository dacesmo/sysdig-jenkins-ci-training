pipeline {
    ...
    stages {
        ...
        stage('Sysdig Vulnerability Scan'){
             steps{
                 script {
                     withCredentials([usernamePassword(credentialsId: 'sysdig-onprem-sa', usernameVariable: 'foo-bar', passwordVariable: 'secure_api_token')]) {
                        ...
                        sh "SECURE_API_TOKEN=${secure_api_token} ./sysdig-cli-scanner --apiurl ${sysdig_url} ${sysdig_cli_args} ${registry_url}/${registry_repo}/${docker_tag} || cat /var/lib/jenkins/workspace/OnPrem-Test/scan-logs && exit 1"
